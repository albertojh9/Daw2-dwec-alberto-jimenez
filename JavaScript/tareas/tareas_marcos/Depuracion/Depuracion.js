...
let p1 = id_alumno * 3n / 7n; // Multiplicación y división con BigInt
p1 += 123n;

let a = p1; // Asigna aquí el valor correcto
if(a != p1) throw "RETO 1 No superado";
console.log("RETO 1 = " + a);


//----------------------------------------------------------------------
// RETO 2. Asigna un valor a b de modo que el resultado sea 666.
// Puedes utilizar el evaluador de expresiones o crear una inspección para
// calcular el valor que tienes que asignar.
//----------------------------------------------------------------------

let b = id_alumno - 666n;
let r1 = id_alumno - b;
if(r1 != 666n) throw "RETO 2 No superado";
...
// En el visor de variables puedes ver el tamaño. También puedes evaluar
...
let tamaño = BigInt(vector.length); // Tamaño convertido a BigInt para operar con n que es BigInt
let n = 0n;
for(; n < tamaño; n++) {
    vector[Number(n)] = n % 128n; // Para indexar en array se usa Number()
}

// Ponte en la siguiente fila ejecutable y pulsa F4 para continuar aquí
if(n != BigInt(vector.length)) throw "RETO 3 No superado";
...
let posición = tamaño / 2n; // División con BigInt
let c = vector[Number(posición)]; // Para indexar en array se usa Number()
if(c != vector[Number(posición)])
...
let ps = Math.floor(Math.random() * s1.length);
...
// Cuando el bucle se encuentre en la iteración indicada por la variable iteración
//
...
function reto6() {
    ...
    for(; n < iteraciones; n++) {
        if(n == iteracion) {
            ...
            valor_anterior = BigInt(Math.random().toFixed());
        }
    }
